...
REAL*8, Parameter :: minTemp = 0.007955 !0.003978 0.5K 0.039777 5 K
REAL*8, Parameter :: maxTemp = 0.318218 !0.397772 50K 0.357995 45K 0.318218 40K 0.278441 35 K 0.238663 - 30 K
Integer, parameter :: numberOfAtoms = 147

Integer, parameter :: initialEquilibriation = 5000
Integer, parameter :: numberOfSwapTriesPriorToRecursion = 100
Integer, parameter :: numberOfBetaRecursions = 10
Integer, parameter :: betaDistributionEq = 1000
...
if(myId.eq.0) then
  open(7,file="ptMpi"//charNumberOfAtoms//"run"//cRD//".log", form="formatted", status="unknown")
...
open(6,file="ptAr"//charNumberOfAtoms//"NData"//charMyId//"run"//cRD//".dat", form="formatted", status="unknown")
...
open(8,file="ptAr"//charNumberOfAtoms//"N"//charMyBetaId//"run"//cRD//".xyz", form="formatted", status="unknown")
close(8)

if(myId.eq.0) then
  open(7,file="ptMpi"//charNumberOfAtoms//"run"//cRD//".log", form="formatted", status="old", access="append")
...
open(6,file="ptAr"//charNumberOfAtoms//"NData"//charMyId//"run"//cRD//".dat", form="formatted", status="old", access="append")
...
do i=0, productionSweeps
    ...
    if(mod(i,loggingFrequency) .eq. 0) then
      LjEns%runningVavg = LjEns%runningVavg/dble(loggingFrequency)
      write(charMyBetaId,'(I2.2)') LjEns%myBetaId
      open(8,file="ptAr"//charNumberOfAtoms//"N"//charMyBetaId//"run"//cRD//".xyz", form="formatted", status="old", access="append")
      write(8,'(I3)') numberOfAtoms
      write(8,'("Energy = ",F12.5," || minimal Energy =",F12.5," at Beta: ",F10.5)') LjEns%runningVavg, minEnergy, betaArray(LjEns%myBetaId)
...
write(6,'("Lowest energy configuration: Energy = ",F12.5)') minEnergy
do i=1, numberOfAtoms
...
!    *******************************************************************
	  ...
	  LjEns%toSend(3)=LjEns%acceptance
          ...
          LjEns%toSendReals(2) = LjEns%maxDisplacement !to test for +1 number of data
          LjEns%toSendReals(3) = LjEns%runningVavg
          
          CALL MPI_SEND(LjEns%toSend,3,MPI_INTEGER,LjEns%betaNeighbours(2),itag,MPI_COMM_WORLD,IERR)
          CALL MPI_SEND(LjEns%toSendReals,3,MPI_REAL8,LjEns%betaNeighbours(2),itagReal,MPI_COMM_WORLD,IERR) ! sending 2 numbers
          CALL MPI_RECV(LjEns%toRecieve,2,MPI_INTEGER,LjEns%betaNeighbours(2),itag,MPI_COMM_WORLD,STATUS,IERR)
          CALL MPI_RECV(LjEns%toRecieveReals,2,MPI_REAL8,LjEns%betaNeighbours(2),itagReal,MPI_COMM_WORLD,STATUS,IERR) ! recieving 1 number back
          ...
          IF(LjEns%toRecieve(1).NE.-2) THEN
            ...
            LjEns%maxDisplacement=LjEns%toRecieveReals(1) ! writing one number
            LjEns%runningVavg=LjEns%toRecieveReals(2)
          END IF
      ...
      IF(MY_IND2.EQ.offset) THEN ! Processes MY_IND2.
        ...
        IF(LjEns%betaNeighbours(1).GE.0) THEN
          ...
          CALL MPI_RECV(LjEns%toRecieveReals,3,MPI_REAL8,LjEns%betaNeighbours(1),itagReal, MPI_COMM_WORLD,STATUS,IERR) ! +1 real for extra data
          ...
          IF(mcCriterionForReplicaSwap(LjEns%staticBetaList(LjEns%myBetaId),LjEns%staticBetaList(LjEns%myBetaId-1),LjEns%V,LjEns%toRecieveReals(1), mts)) THEN
            ...
            LjEns%toSendReals(1)=LjEns%maxDisplacement ! +1 data unit 
            LjEns%toSendReals(2)=LjEns%runningVavg ! +2 data unit
            ...
            LjEns%maxDisplacement=LjEns%toRecieveReals(2)
            LjEns%runningVavg=LjEns%toRecieveReals(3)
            LjEns%myBetaId=LjEns%myBetaId-1
          ELSE
            LjEns%toSend(1)=-2
          END IF
          ...
          CALL MPI_SEND(LjEns%toSendReals,2,MPI_REAL8,NDEST1,itagReal, MPI_COMM_WORLD,IERR)
        END IF
