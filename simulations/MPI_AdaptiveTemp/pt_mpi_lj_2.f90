...
REAL*8, Parameter :: minTemp = 0.003978 !0.039777 5 K
REAL*8, Parameter :: maxTemp = 0.357995 !0.397772 50K 0.357995 45K 0.318218 40K 0.278441 35 K 0.238663 - 30 K
Integer, parameter :: numberOfAtoms = 75

Integer, parameter :: initialEquilibriation = 100000
Integer, parameter :: numberOfSwapTriesPriorToRecursion = 500
...
do i=0, productionSweeps
    ...
    call sweepOverReplicaWOaccept( numberOfAtoms, LjEns, myMts )
...
!    *******************************************************************
	  ...
	  LjEns%toSend(3)=LjEns%acceptance
          ...
          CALL MPI_SEND(LjEns%toSendReals,2,MPI_REAL8,LjEns%betaNeighbours(2),itagReal,MPI_COMM_WORLD,IERR) ! sending 2 numbers
          CALL MPI_RECV(LjEns%toRecieve,2,MPI_INTEGER,LjEns%betaNeighbours(2),itag,MPI_COMM_WORLD,STATUS,IERR)
          CALL MPI_RECV(LjEns%toRecieveReals,1,MPI_REAL8,LjEns%betaNeighbours(2),itagReal,MPI_COMM_WORLD,STATUS,IERR) ! recieving 1 number back
      ...
      IF(MY_IND2.EQ.offset) THEN ! Processes MY_IND2.
        ...
        IF(LjEns%betaNeighbours(1).GE.0) THEN
          ...
          CALL MPI_RECV(LjEns%toRecieveReals,2,MPI_REAL8,LjEns%betaNeighbours(1),itagReal, MPI_COMM_WORLD,STATUS,IERR) ! +1 real for extra data
          ...
          CALL MPI_SEND(LjEns%toSendReals,1,MPI_REAL8,NDEST1,itagReal, MPI_COMM_WORLD,IERR)
        END IF
