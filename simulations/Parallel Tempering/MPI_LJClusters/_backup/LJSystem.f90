...
!    		*******************************************************************	
		Subroutine sweepOverReplica ( N, SIGMA, DRMAX, LjEns )
...
!          	** MOVE I **
              	...
              	RXINEW = RXIOLD + ( 2.0*rmafun() - 1.0 ) * DRMAX
              	RYINEW = RYIOLD + ( 2.0*rmafun() - 1.0 ) * DRMAX
              	RZINEW = RZIOLD + ( 2.0*rmafun() - 1.0 ) * DRMAX
...
!    *******************************************************************
	...
	SUBROUTINE InitLjSystemBeta ( LjEns, Beta, higherBetaId, myBetaId, lowerBetaId )
...
!    *******************************************************************
	...
	LjEns%higherBetaId = higherBetaId 
        ...
        SUBROUTINE tryReplicaSwap ( numberOfProcesses, LjEns, myId )
...
!    *******************************************************************
      ...
      integer itag, toSnd, toRcv
      integer, dimension(3) :: toSend
      integer, dimension(3) :: toRecieve
...
!	** Create local list of neighbours [2 colder, this process, 1 hotter]
	...
	if (myId .ne. numberOfProcesses ) then
	  toSnd = LjEns%higherBetaId
	  call MPI_SEND(toSnd, 1, MPI_INTEGER, LjEns%lowerBetaId, itag, MPI_COMM_WORLD, ierr)
	end if
	if (myId .ne. 0) then
	  call MPI_RECV(toRcv, 1, MPI_INTEGER, LjEns%higherBetaId, itag, MPI_COMM_WORLD, MPI_STATUS_IGNORE)
	  LjEns%neighbourList(1) = toRcv
	end if
